...
edit_proj_field = State()
edit_proj_value = State()
edit_proj_id = State()
...
InlineKeyboardButton(text="📋 Loyihalar Ro'yxati", callback_data="adm_list_proj")],
...
@dp.callback_query(F.data == "adm_list_proj")
async def adm_list_projects(callback: types.CallbackQuery):
projs = db_query("SELECT id, name FROM projects", fetchall=True)
if not projs:
    return await callback.message.answer("📂 Hozircha loyihalar yuklanmagan.")
kb = []
for pid, name in projs:
    kb.append([
        InlineKeyboardButton(text=f"📁 {name}", callback_data=f"adm_view_proj_{pid}"),
        InlineKeyboardButton(text="✏️ Tahrirlash", callback_data=f"adm_edit_proj_{pid}"),
        InlineKeyboardButton(text="🗑 O'chirish", callback_data=f"adm_del_proj_{pid}")
    ])
await callback.message.answer("📋 Loyihalar ro'yxati:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
await callback.answer()

@dp.callback_query(F.data.startswith("adm_view_proj_"))
async def adm_view_project(callback: types.CallbackQuery):
pid = int(callback.data.split("_")[-1])
proj = db_query("SELECT name, price, description FROM projects WHERE id = ?", (pid,), fetchone=True)
if not proj:
    return await callback.answer("Loyiha topilmadi.", show_alert=True)
name, price, desc = proj
msg = f"📂 **{name}**\n💰 Narx: {price}\n📝 {desc}"
await callback.message.answer(msg, parse_mode="Markdown")
await callback.answer()

@dp.callback_query(F.data.startswith("adm_del_proj_"))
async def adm_delete_project_confirm(callback: types.CallbackQuery):
pid = int(callback.data.split("_")[-1])
kb = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text="✅ Ha, o'chirish", callback_data=f"confirm_del_proj_{pid}"),
     InlineKeyboardButton(text="❌ Yo'q", callback_data="cancel_del")]
])
await callback.message.answer(f"Loyiha ID: {pid} ni o'chirishni xohlaysizmi?", reply_markup=kb)
await callback.answer()

@dp.callback_query(F.data.startswith("confirm_del_proj_"))
async def adm_delete_project(callback: types.CallbackQuery):
pid = int(callback.data.split("_")[-1])
db_query("DELETE FROM projects WHERE id = ?", (pid,), commit=True)
await callback.message.answer("✅ Loyiha o'chirildi!")
await callback.answer()
...
@dp.callback_query(F.data.startswith("adm_edit_proj_"))
async def adm_edit_project_start(callback: types.CallbackQuery, state: FSMContext):
pid = int(callback.data.split("_")[-1])
proj = db_query("SELECT id FROM projects WHERE id = ?", (pid,), fetchone=True)
if not proj:
    return await callback.answer("Loyiha topilmadi.", show_alert=True)
await state.update_data(edit_pid=pid)
kb = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text="📝 Nom", callback_data="edit_field_name")],
    [InlineKeyboardButton(text="💰 Narx", callback_data="edit_field_price")],
    [InlineKeyboardButton(text="🗒 Description", callback_data="edit_field_desc")],
    [InlineKeyboardButton(text="🖼 Media", callback_data="edit_field_media")],
    [InlineKeyboardButton(text="📁 Fayl", callback_data="edit_field_file")]
])
await callback.message.answer("Qaysi maydonni tahrirlashni xohlaysiz?", reply_markup=kb)
await state.set_state(AdminState.edit_proj_field)
await callback.answer()

@dp.callback_query(StateFilter(AdminState.edit_proj_field), F.data.startswith("edit_field_"))
async def adm_select_edit_field(callback: types.CallbackQuery, state: FSMContext):
field = callback.data.split("_")[-1]
await state.update_data(edit_field=field)
if field in ["name", "desc"]:
    await callback.message.answer(f"Yangi {field.upper()} ni yozing:", reply_markup=cancel_kb())
elif field == "price":
    await callback.message.answer(f"Yangi narxni yozing ({CURRENCY_SYMBOL}):", reply_markup=cancel_kb())
elif field == "media":
    await callback.message.answer("Yangi rasm yoki videoni yuboring (yoki 'skip' deb yozing):", reply_markup=cancel_kb())
elif field == "file":
    await callback.message.answer("Yangi faylni yuboring:", reply_markup=cancel_kb())
await state.set_state(AdminState.edit_proj_value)
await callback.answer()

@dp.message(AdminState.edit_proj_value)
async def adm_save_edit_value(message: types.Message, state: FSMContext):
data = await state.get_data()
pid = data['edit_pid']
...
if field in ["name", "desc"]:
    ...
    try:
        ...
        return await message.answer("⚠️ Raqam yozing!")
elif field == "media":
    mid, mtype = None, None
    if message.photo:
    ...
    db_query(f"UPDATE projects SET media_id = ?, media_type = ? WHERE id = ?", (mid, mtype, pid), commit=True)
    await message.answer("✅ Media yangilandi!", reply_markup=main_menu(message.from_user.id))
    await state.clear()
    return
elif field == "file":
    if not message.document:
        return await message.answer("⚠️ Fayl yuboring!")
    value = message.document.file_id
    field = "file_id"

if value is not None:
    db_query(f"UPDATE projects SET {field} = ? WHERE id = ?", (value, pid), commit=True)
    await message.answer(f"✅ {field.upper()} yangilandi!", reply_markup=main_menu(message.from_user.id))
await state.clear()
